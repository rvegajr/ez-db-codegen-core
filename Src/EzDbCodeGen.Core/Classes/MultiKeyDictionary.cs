...
namespace EzDbCodeGen.Core
{
	...
	public class MultiKeyDictionary<K, L, V>
	{
        ...
        protected  Dictionary<K, L> primaryToSubkeyMapping = new Dictionary<K, L>();

        protected ReaderWriterLockSlim readerWriterLock = new ReaderWriterLockSlim();
        ...
        /// <summary>
        /// Tries the get the value of the associate key
        /// </summary>
        /// <returns><c>true</c>, if get value was tryed, <c>false</c> otherwise.</returns>
        /// <param name="subKey">Sub key.</param>
        /// <param name="associateKey">Associate Key.</param>
        public virtual bool TryGetValue(L subKey, out K associateKey)
        {
            associateKey = default(K);

            readerWriterLock.EnterReadLock();

            try
            {
                if (subDictionary.TryGetValue(subKey, out associateKey))
                {
                    return true;
                }
            }
		...
		public virtual bool TryGetValue(L subKey, out V val)
		{
			val = default(V);
        ...
        public virtual bool TryGetValue(K primaryKey, out V val)
		{
			readerWriterLock.EnterReadLock();
        ...
        public bool ContainSubKey(L subKey)
        {
            V val;
