...
namespace EzDbCodeGen.Core.Extentions
{
    ...
    /// </summary>
    internal static class PropertyExtentions
    {
        /// <summary>
        ...
        {
            if (This.CustomAttributes == null) This.CustomAttributes = new EzDbSchema.Core.Objects.CustomAttributes();
        ...
        internal static bool IsNullableResolved(this IProperty This)
        {
            ...
            var firstConfigEntity = Internal.AppSettings.Instance.Configuration.FindMatchingConfigEntities(new SchemaObjectName(This.ParentEntity).AsFullName()).ToList().FirstOrDefault();
            if (firstConfigEntity != null)
            {
                var FieldOverride = firstConfigEntity.Overrides.Fields.Find(f => f.FieldName.ToLower().Equals(This.PropertyName.ToLower()));
        ...
        {
			return This.ParentEntity.PrimaryKeys.AsParmString(prefix, delimiter, elementSet);
        }
        ...
        {
			return This.ParentEntity.PrimaryKeys.AsLinqEquationString(prefix, delimiter, elementSet, prefixSetter);
        }
		...
		public static string AsODataRouteString(this IProperty This) {
			return This.ParentEntity.PrimaryKeys.AsODataRouteString(); 
		}
		...
		public static string AsODataRouteString(this IPrimaryKeyProperties This )
        {
            ...
            else
            {
                ...
                {
					var property = This[i];
                    ret += (i > 0 ? @", " : @" ") + property.ColumnAlias + "={" + property.ColumnAlias + "}";
                }
            }
            return "(" + ret + ")";
        }
		...
		{
			return This.ParentEntity.PrimaryKeys.AsCsvString(varPrefix, true);
		}
        ...
        {
            return This.ParentEntity.PrimaryKeys.AsCsvString(varPrefix, AsObjectPropertyName);
        }
		...
		public static string AsCsvString(this IPrimaryKeyProperties This, string varPrefix, bool AsObjectPropertyName)
        {
            ...
            {
				var property = This[i];
                ret += (i > 0 ? @", " : @" ") + ((varPrefix.Length > 0) ? varPrefix + "." : "") + ((AsObjectPropertyName) ? property.AsObjectPropertyName() : property.ColumnAlias);
            }
            return ret;
        }
		...
		{
			return This.ParentEntity.PrimaryKeys.AsParmBooleanCheck(varPrefix, op);
		}

        public static string AsParmBooleanCheck(this IProperty This, string varPrefix, string op, bool UseObjectPropertyName)
        {
            return This.ParentEntity.PrimaryKeys.AsParmBooleanCheck(varPrefix, op, UseObjectPropertyName);
        }
		...
		public static string AsParmBooleanCheck(this IPrimaryKeyProperties This, string varPrefix, string op, bool UseObjectPropertyName)
        {
            ...
            {
				var property = This[i];
                ret += (i > 0 ? @" && " : @"") + property.ColumnAlias + " " + op.Trim() + " " + varPrefix + "." + ((UseObjectPropertyName) ? property.AsObjectPropertyName() : property.ColumnAlias);
            }
            return ret;
        }
        ...
        public static string AsParmBooleanCheck(this IPrimaryKeyProperties This, string varPrefix, string op)
        {
            ...
            {
                var property = This[i];
                ret += (i > 0 ? @" && " : @"") + property.ColumnAlias + " " + op.Trim() + " " + varPrefix + "." + property.AsObjectPropertyName();
            }
            return ret;
        }
        ...
        public static string AsObjectPropertyName(this IProperty property, string nameCollisionSuffix)
        {
            ...
            var propertyName = property.ColumnAlias.ToCsObjectName();
            try
            {
                var entity = property.GetType().GetProperty("ParentEntity")?.GetValue(property) as IEntity;
                if (entity == null) return property.PropertyName;
                var database = entity.ParentDatabase;
                var entityName = entity.TableName ?? string.Empty;
                //debugging line
                ...
                if (parentEntity?.Relationships.FindItems(RelationSearchField.ToColumnName, columnAlias).Count >= 1)
                {
                    propertyName += nameCollisionSuffix;
                } else if (entity.TableAlias.Equals(propertyName)) {
                    propertyName += nameCollisionSuffix;
                }
            }
            catch (Exception ex)
            {
                var parentEntity = property.GetType().GetProperty("ParentEntity")?.GetValue(property) as IEntity;
                var propName = property.GetType().GetProperty("Name")?.GetValue(property) as string ?? string.Empty;
                throw new Exception(string.Format("{0}: Error while figuring out property name for {1}.{2}", PROC_NAME, parentEntity?.TableName ?? "<unknown>", propName), ex);
            }
            return propertyName;
        }
    }
