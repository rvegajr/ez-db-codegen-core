...
public static class CommandMain
{
    ...
    static void InteractiveConnectionString()
    {
        ...
        Console.ResetColor();
        var defaultConnectionString = "Server=localhost;Database=YourDatabaseName;Trusted_Connection=True;TrustServerCertificate=True;";
        var connparm = new EzDbSchema.MsSql.ConnectionParameters();
        ...
        if ((ConnectionStringLocal ?? "") == "")
        {
            while (questionLoop)
            {
                ...
                connparm.TrustServerCertificate = Prompt.GetYesNo("Would you like to automatically trust the server certificate [y/n]?  (fine for local db instances)", true, promptColor: ConsoleColor.Green);
        ...
        app.OnExecute(() =>
        {
            ...
            try
            {
                ...
                if (Settings.AutoRun)
                {
                    ...
                    AppSettings.Instance.VerboseMessages = Settings.Verbose;
                }
                ...
                {
                    if ((Settings.Config != string.Empty) && (File.Exists(Settings.Config)))
                    {
                        Console.WriteLine($"Config passed through settings json file and exists at {Settings.Config}");
                        AppSettings.Instance.ConfigurationFileName = Settings.Config;
                    }
                    else
                    {
                        var configFileFound = Path.Combine(Environment.CurrentDirectory, "ezdbcodegen.config.json");
                        var appConfigFile = Path.Combine(Environment.CurrentDirectory, $"{Settings.AppName}.config.json");
                        string[] ConfigurationFileNameSearchList = Directory.GetFiles(Path.Combine(Environment.CurrentDirectory, "").PathEnds(), "*.config.json");
                        if (ConfigurationFileNameSearchList.Contains(appConfigFile, StringComparer.InvariantCultureIgnoreCase))
                        {
                            Console.WriteLine($"Found Config file at {appConfigFile}");
                            AppSettings.Instance.ConfigurationFileName = appConfigFile;
                        }
                        else if (ConfigurationFileNameSearchList.Contains(configFileFound, StringComparer.InvariantCultureIgnoreCase))
                        {
                ...
                Console.WriteLine(pfx + "CodeGenerator Versions: ");
                Console.WriteLine(pfx + "   SchemaCoreVersion " + AppSettings.Instance.SchemaCoreVersion);
                Console.WriteLine(pfx + "  SchemaMssqlVersion " + AppSettings.Instance.SchemaMssqlVersion);
                Console.WriteLine(pfx + "   CodeGenCliVersion " + AppSettings.Instance.CodeGenCliVersion);
                Console.WriteLine(pfx + "  CodeGenCoreVersion " + AppSettings.Instance.CodeGenCoreVersion);
